...
# Strict .co mode: all available alerts are routed to the fixed topic below.
TELEGRAM_TOPIC_ID = 122946
PRIMARY_TLD = ".co"
PRIORITY_TLDS = frozenset({PRIMARY_TLD})
...
STATE_FILE_SUFFIX = "_co"
ALERT_DB_FILENAME = f"alerts{STATE_FILE_SUFFIX}.db"
PROCESSED_CSV_FILENAME = f"processed_domains{STATE_FILE_SUFFIX}.csv"
PROCESSED_CSV_LOCK = threading.Lock()
PROCESSED_CSV_PATH = Path(__file__).with_name(PROCESSED_CSV_FILENAME)
...
class WatcherConfig:
    ...
    allowed_tlds: set[str] = field(default_factory=lambda: set(TARGET_TLDS))
    ...
    def from_env(cls) -> "WatcherConfig":
        ...
        return cls(
            ...
            db_path=os.getenv("ALERT_DB_PATH_CO", ALERT_DB_FILENAME),
...
def _sanitize_strict_tld_domain(raw_domain: Any) -> str:
    ...
    Rules:
    - no whitespace
    - exactly one trailing target TLD extension
    ...
    if not clean_domain.endswith(strict_tld):
        return ""
    if clean_domain.count(strict_tld) != 1:
        return ""
    keyword = clean_domain[: -len(strict_tld)]
...
class SpaceshipClient:
    ...
    Bulk availability endpoint:
        ...
        Body: {"domains": ["example.co", ...]}   (max 20 per call)
...
def log_to_processed_csv(
    ...
    """
    Persist per-domain processing result to processed_domains_co.csv.
...
def load_processed_available_domains() -> set[str]:
    """
    Load processed_domains_co.csv and return domains previously marked as Available.
...
async def fetch_spaceship_domains(app: Application) -> dict[str, int]:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            priority_count = sum(1 for d in selected_domains if d.endswith(PRIMARY_TLD))
            LOGGER.info(
                "Priority coverage this cycle: %s=%s (total=%s)",
                PRIMARY_TLD,
                priority_count,
